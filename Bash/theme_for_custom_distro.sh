...
function themes () {
	...
	cp -f $DIR/Flat-Remix.tar.xz /usr/share/icons/
	tar -xf /usr/share/icons/Flat-Remix.tar.xz -C /usr/share/icons/

	# Installs McMojave-circle-black icon pack
	cp -f $DIR/McMojave-circle-black.tar.xz /usr/share/icons/
	tar -xf /usr/share/icons/McMojave-circle-black.tar.xz -C /usr/share/icons/

	# Installs McOS-Dark Theme
	if [[ `de` == 'xfce' ]]; then
		cp -f $DIR/McOS-MJV-Dark-XFCE-Edition-2.3.tar.gz /usr/share/themes/
		tar -xf /usr/share/themes/McOS-MJV-Dark-XFCE-Edition-2.3.tar.gz -C /usr/share/themes/
		...
		rm /usr/share/themes/McOS-MJV-Dark-XFCE-Edition-2.3.tar.gz
	elif [[ `de` == 'gnome' ]]; then
		cp -f $DIR/Mc-OS-MJV-Dark-Gn3.32-V.2.1.tar.xz /usr/share/themes/
		tar -xf /usr/share/themes/Mc-OS-MJV-Dark-Gn3.32-V.2.1.tar.xz -C /usr/share/themes/
		# Clean the Directories
		if [[ $DIR == '.' ]]; then
		...
		rm /usr/share/themes/Mc-OS-MJV-Dark-Gn3.32-V.2.1.tar.xz
	fi
	
	# The Walpaper
	cp $DIR/xubuntu-development.png /usr/share/backdrops

	# Clean the Directories
	rm /usr/share/icons/Flat-Remix.tar.xz
}
